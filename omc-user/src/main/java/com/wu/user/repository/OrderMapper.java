...
public interface OrderMapper extends BaseMapper<Order> {
    ...
    @Override
    @Update("update onlinemallcake.order set total = #{total}, amount = #{amount}, status = #{status}, pay_type = #{payType}, name = #{name}, phone = #{phone}, address = #{address}, datetime = #{datetime}, user_id = #{userId} where id = #{id}")
    Boolean updateByPrimaryKey(Order record);

    @Update("update onlinemallcake.order set total = #{total}, amount = #{amount}, status = #{status}, pay_type = #{payType}, name = #{name}, phone = #{phone}, address = #{address}, datetime = #{datetime} where user_id = #{userId}")
